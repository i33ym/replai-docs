---
title: "Play Store"
description: "Подключение и настройка Play Store с Replai."
weight: 11
---

## Обзор

Это руководство поможет вам создать сервисный аккаунт Google Cloud, сгенерировать файл учётных данных JSON и подключить его к приложению в Google Play Console, чтобы Replai мог читать отзывы вашего приложения и отвечать на них.
...
Введите имя для сервисного аккаунта (например, `replai-playstore`) и нажмите **Continue**.
...
Файл учётных данных JSON будет автоматически загружен на ваш компьютер. Сохраните этот файл в надёжном месте — вам нужно будет загрузить его в Replai.
...
## 7. Подключение к Replai

### Шаг 29: Откройте настройки Replai

Перейдите в Replai и откройте **Settings** → **Integrations**.

![Открыть настройки Replai](/images/play-store/29-open-replai-settings.png)
...
Вы увидите подтверждение того, что Play Store подключён к Replai.
